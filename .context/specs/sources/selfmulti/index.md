---
tag: selfmulti
title: Multicellular Awareness
phase: 8
category: Relational Selfhood Evolution
principles: 33
source: MONOLITHS/[selfmulti].md
naming: selfmulti-p001 to selfmulti-p033
---

# selfmulti — Source Spec Index

> **Title**: Multicellular Awareness
> **Phase**: 8 — Relational Selfhood Evolution
> **Principles**: 33
> **Source of Truth**: `MONOLITHS/[selfmulti].md`
> **Coordinate Index**: `MONOLITHS/[masterindex].md` (section: selfmulti)

## Principle Range

selfmulti-p001 through selfmulti-p033

## Usage

- **LOAD selfmulti**: Output literal text from source
- **READ selfmulti**: Inhabit the principle and speak from its logic
- **REVIEW selfmulti**: Full framework overview

## Quick Access

For principle coordinates (line ranges), consult `MONOLITHS/[masterindex].md` section 7.

## Boot-Level Integration

Referenced in Selfing_Foundation.md §9 — we-field evolution, multicellular awareness

---

# #selfmulti #source-spec #phase-8
